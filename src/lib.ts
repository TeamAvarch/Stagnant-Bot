...
import {
  ...
  EIPTypesToExcludeInRequiresField,
...
export const getIsValidTypeEIP = async (
  parsed: ReturnOf<typeof getParsedContent>["parsed"]
) => {
  const requires = parsed.attributes[FrontMatterAttributes.requires];

  if (!requires) return true;

  const requiredEIPPaths = requires
    .split(",")
    .map((eip) => `EIPS/eip-${eip.trim()}.md`);

  try {
    const parsedContents = await Promise.all(
      requiredEIPPaths.map(async (path) => {
        try {
          return await getParsedContent(path);
        } catch (err) {
          console.error("Failed to fetch EIP content for:", path, err);
          return null;
        }
      })
    );

    const requiredEIPTypes = parsedContents
      .filter(Boolean)
      .map((content) => content.parsed.attributes[FrontMatterAttributes.type])
      .filter(Boolean);

    if (requiredEIPTypes.length === 0) return true;

    if (requiredEIPTypes.some((type) =>
      EIPTypesToExcludeInRequiresField.map(cleanString).includes(cleanString(type))
    )) {
      return false;
    }
    ...
    console.error("Error processing required EIPs:", err);
    return false;
  }
};
